 ...
 * 图书分类接口。
 *
 * <p><b>为什么挂在 {@code /api/books/categories} 而不是 {@code /api/categories}</b>:
 * {@code JwtAuthFilter} 的白名单是一条前缀规则 {@code ("/api/books", GET)}——
 * 只要路径落在 {@code /api/books} 下面且是 GET,未登录的商城访客就能读。
 * 换个前缀就要再往白名单里加一条,读接口的形状没必要和鉴权配置耦合。</p>
 *
 * <p><b>会不会和 {@code GET /api/books/{isbn}} 撞车</b>:不会。Spring 的路径匹配
 * 里字面量段 {@code categories} 的优先级高于变量段 {@code {isbn}},
 * 所以 {@code /api/books/categories} 永远进这里,不会被当成 isbn="categories"。</p>
 */
@Slf4j
@RestController
...
public class CategoryController {
    ...
    /**
     * 分类树(两级,含每类的图书数)—— GET /api/books/categories
    ...
    /**
     * 新建分类 —— POST /api/books/categories
