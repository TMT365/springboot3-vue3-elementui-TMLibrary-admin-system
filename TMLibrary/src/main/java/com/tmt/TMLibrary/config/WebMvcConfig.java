 ...
 * Spring MVC 通用配置。
 *
 * <p>当前仅注册 CORS：放行前端 dev server (Vue Vite, 默认 :5173) 对 {@code /api/**}
 * 的跨域请求。Spring Security 等拦截器未引入，所以直接用 {@code WebMvcConfigurer}
 * 即可，不需要 filter chain。</p>
 *
 * <p>允许的 origin 通过 {@code app.cors.origins} 配置(逗号分隔),默认 {@code http://localhost:5173}。
 * 生产环境按域名收敛,不要用 {@code *} —— 那会连 credentials 一起被禁掉。</p>
 */
@Configuration
